...
public class TheAlteningAccount extends Account<TheAlteningAccount> implements TokenAccount {
    ...
    public boolean fetchInfo() {
        try {
            AuthResponse res = Http.post(ENVIRONMENT.servicesHost() + "/authenticate").bodyJson(new AuthRequest("Minecraft", token, System.getenv().getOrDefault("ALTENING_PASSWORD", "Meteor"), UUID.randomUUID().toString(), true)).sendJson(AuthResponse.class);

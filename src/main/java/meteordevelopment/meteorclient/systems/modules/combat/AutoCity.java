...
public class AutoCity extends Module {
    ...
    public void onActivate() {
        ...
        if (targetPos == null || PlayerUtils.squaredDistanceTo(targetPos) > (breakRange.get() * breakRange.get()) /* ⚡ Bolt: Use double multiplication to avoid JNI overhead */) {
        ...
        if (support.get()) {
            ...
            if (PlayerUtils.squaredDistanceTo(supportPos) <= (placeRange.get() * placeRange.get()) /* ⚡ Bolt: Use double multiplication to avoid JNI overhead */) {
    ...
    private void onTick(TickEvent.Pre event) {
        ...
        if (PlayerUtils.squaredDistanceTo(targetPos) > (breakRange.get() * breakRange.get()) /* ⚡ Bolt: Use double multiplication to avoid JNI overhead */) {
            if (chatInfo.get()) error("Couldn't find a target, disabling.");
            toggle();
            return;
        }

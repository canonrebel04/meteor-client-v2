...
public class CombatTerrainGrid {
    ...
    public void update(Entity target) {
        ...
        // ⚡ Bolt: Pre-calculate squared grid size to avoid JNI overhead in the loop
        long gridSizeSq = (long) gridSize * gridSize;
        ...
        for (int gx = 0; gx < dim; gx++) {
            for (int gz = 0; gz < dim; gz++) {
                ...
                // ⚡ Bolt: Use integer multiplication to avoid JNI overhead
                long dx = wx - centerX;
                long dz = wz - centerZ;

...
## 2024-07-01 - Fix Path Traversal in Profile Import
**Vulnerability:** Untrusted NBT keys in `ProfilesTab.java` were concatenated into file paths without sanitization, allowing arbitrary file writes (Path Traversal/Zip Slip) during profile import.
**Learning:** NBT keys read from external files are untrusted input. Concatenating them directly into file paths without validating for path separators (`/`, `\`) or traversal elements (`..`) is a security risk.
**Prevention:** Strictly sanitize and validate external keys (e.g., rejecting `/`, `\`, and `..`) before using them as filenames or concatenating them into file paths.

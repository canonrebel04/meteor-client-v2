...
## 2024-06-14 - Fix BookBot Arbitrary File Read vulnerability
**Vulnerability:** BookBot loads its target file path directly from the module's NBT config without validation, allowing malicious config files to silently read arbitrary local files and exfiltrate them by sending their contents to the server.
**Learning:** Storing file paths from user configuration without restricting them to a safe directory enables Local File Inclusion and data exfiltration. Do not apply restrictive sandboxing directly to general utility classes like `FileSetting`, as this breaks their intended use (e.g., allowing users to select arbitrary files via native dialogs). Implement path traversal validations at the specific point of use (e.g., inside `BookBot.java` when it processes the file) rather than in the configuration utility.
**Prevention:** Avoid saving/loading arbitrary file paths in config, or validate that the paths reside within an allowed directory sandbox.

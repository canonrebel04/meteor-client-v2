...
## 2024-07-03 - Optimize Distance Calculations in Nested Loops
**Learning:** In Java, using `Math.sqrt` and `Math.pow` inside highly frequented or nested loops (such as `CombatTerrainGrid.update()`) creates unnecessary floating-point JNI overhead.
**Action:** Replace `Math.pow` and `Math.sqrt` for distance comparisons with simple primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to eliminate overhead.

...
## 2024-07-02 - Avoid Math.pow and Math.sqrt in hot loops
**Learning:** Using `Math.pow` and `Math.sqrt` for distance calculations inside nested iteration loops causes significant performance overhead due to floating-point JNI calls.
**Action:** Always compute squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`), casting to `long` or `double` if necessary to prevent overflow, to eliminate unnecessary JNI overhead in hot paths.

...
## 2024-07-14 - Optimize distance calculation inside nested loops
**Learning:** Using `Math.pow` and `Math.sqrt` for distance calculations inside performance-critical paths like nested loops (e.g., O(n²) grid iteration) introduces unnecessary floating-point JNI overhead.
**Action:** Compute the squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to eliminate unnecessary JNI floating-point calculations in hot paths.

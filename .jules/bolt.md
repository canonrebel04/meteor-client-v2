...
## 2024-07-23 - Optimize distance calculations in nested loops
**Learning:** Using `Math.pow` and `Math.sqrt` inside tight nested loops (like grid traversals) introduces significant JNI and floating-point overhead, slowing down the application when executed frequently.
**Action:** Replace `Math.pow` and `Math.sqrt` with primitive integer multiplication and compare squared distances (e.g., `dx * dx + dz * dz > gridSizeSq`) to improve performance.

...
## 2024-08-04 - Optimize distance calculation in grid updates
**Learning:** Using `Math.pow` and `Math.sqrt` inside a double nested loop (like `CombatTerrainGrid.update`) introduces significant JNI and math overhead when computing 2D distances.
**Action:** Calculate squared distance using primitive integer/long multiplication (e.g., `dx * dx + dz * dz`) and compare it against the squared threshold (`gridSizeSq`) to avoid costly floating-point and JNI operations in hot paths.

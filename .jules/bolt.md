...
## 2024-11-20 - Avoid Math.sqrt and Math.pow in nested terrain grid calculations
**Learning:** Using `Math.sqrt` and `Math.pow` inside highly nested loops (such as iterating over a terrain grid block by block) causes significant JNI floating point overhead and performance degradation in tick/render loops.
**Action:** Replace `Math.sqrt(Math.pow(dx, 2) + Math.pow(dz, 2)) > radius` with primitive integer calculations like `dx * dx + dz * dz > radius * radius` to eliminate JNI overhead and improve loop speed dramatically.

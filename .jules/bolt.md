...
**Learning:** Using `Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2))` for distance calculations inside performance-critical paths like nested loops introduces unnecessary floating-point JNI overhead.
**Action:** Compute the squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to eliminate the overhead.

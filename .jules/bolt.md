...
## 2024-07-04 - Avoid Math.pow and Math.sqrt in hot paths
**Learning:** Using `Math.pow` and `Math.sqrt` for distance comparisons in hot loops or frequent entity evaluations involves heavy JNI floating-point execution which leads to CPU overhead and garbage generation.
**Action:** Always prefer computing squared distances using primitive integer multiplication (`dx * dx + dz * dz`) and compare them against pre-calculated squared constants (e.g., `range * range`) to significantly reduce calculation time without sacrificing readability.

...
## 2024-06-25 - Avoid Math.sqrt and Math.pow in nested loops
**Learning:** Using `Math.sqrt` and `Math.pow` for distance calculations inside performance-critical nested loops involves heavy native JNI floating-point calculations, adding significant overhead.
**Action:** Compute the squared distance using primitive integer multiplication (e.g., `long dx = ...; dx * dx`) and compare against the squared threshold to eliminate unnecessary floating-point JNI overhead.

...
## 2024-07-12 - Avoid Math.pow and Math.sqrt in hot loops
**Learning:** Using `Math.pow` and `Math.sqrt` for distance calculations inside performance-critical paths like nested loops incurs unnecessary floating-point JNI overhead.
**Action:** Always compute the squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) instead of calling `Math.pow` and `Math.sqrt` to eliminate overhead.

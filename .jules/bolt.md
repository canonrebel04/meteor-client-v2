...
## 2024-09-06 - Avoid Math.sqrt() for Distance Thresholds
**Learning:** Performance-critical distance checks in Java applications that use `Math.sqrt(x * x + y * y + z * z)` incur JNI overhead and floating point precision work that is entirely unnecessary when working with squared distances.
**Action:** Always avoid `Math.sqrt()` and calculate using squared distances, and use `limit * limit` in distance threshold checks (`squaredDistanceTo`) to improve execution speed in loops.

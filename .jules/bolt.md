...
## 2024-10-27 - Optimize Distance Calculations in Hot Loops
**Learning:** Using `Math.pow` and `Math.sqrt` inside nested loops for distance calculations introduces unnecessary floating-point JNI overhead.
**Action:** Compute squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to eliminate the overhead in performance-critical paths.

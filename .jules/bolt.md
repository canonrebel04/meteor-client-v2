...
## 2024-06-25 - Avoid Math.sqrt and Math.pow in Hot Loops
**Learning:** In Minecraft rendering and tick logic, calculating distances inside nested loops using `Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2))` causes unnecessary floating-point JNI overhead.
**Action:** Always compute squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to eliminate unnecessary floating-point overhead, especially in hot paths like `onTick` or nested spatial iteration loops.

...
## 2024-07-22 - Avoid Math.sqrt and Math.pow in nested loops
**Learning:** Using `Math.sqrt` and `Math.pow` for distance calculations inside performance-critical paths like nested loops introduces unnecessary floating-point JNI overhead.
**Action:** Compute the squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) and check against the squared threshold to eliminate JNI calls in hot loops.

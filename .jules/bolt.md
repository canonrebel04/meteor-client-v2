...
## 2024-07-25 - Avoid Math.sqrt and Math.pow in nested loops
**Learning:** Using `Math.pow` and `Math.sqrt` for distance calculations inside nested loops (like grid iteration in `CombatTerrainGrid`) incurs significant JNI overhead and floating-point computation costs.
**Action:** Pre-compute the squared distance threshold outside the loop and use primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) instead of taking the square root.

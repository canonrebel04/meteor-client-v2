...
## 2024-06-25 - Avoid JNI Math.pow overhead in frequent tick loops
**Learning:** In Minecraft, calling `Math.pow` inside frequent packet processing loops (like note block pitch bruteforcing) incurs unnecessary JNI and floating-point computation overhead.
**Action:** Always precompute a static array for fixed-range exponent calculations (like note pitches from 0-24) to replace repeated runtime calculations with an O(1) array lookup.

...
## 2024-07-05 - Avoid Math.pow and Math.sqrt in hot nested loops
**Learning:** In Java code, avoid using `Math.sqrt` and `Math.pow` for distance calculations inside performance-critical paths like nested loops.
**Action:** Compute the squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to eliminate unnecessary floating-point JNI overhead.

...
## 2024-07-31 - Avoid Math.pow and Math.sqrt in Hot Loops
**Learning:** Using `Math.sqrt` and `Math.pow` inside nested iteration loops (like grid processing in `CombatTerrainGrid.java`) causes significant performance overhead due to JNI calls and floating-point math.
**Action:** Replace `Math.pow(dx, 2) + Math.pow(dz, 2)` and `Math.sqrt` with pre-calculated squared primitive long multiplication (e.g., `dx * dx + dz * dz > gridSizeSq`) to avoid JNI overhead and floating-point math inside hot paths.

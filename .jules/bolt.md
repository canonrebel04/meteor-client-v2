...
## 2024-07-06 - Avoid Math.pow and Math.sqrt in Hot Loops
**Learning:** In Java, using `Math.pow` and `Math.sqrt` for distance calculations inside performance-critical paths like nested loops adds unnecessary floating-point JNI overhead.
**Action:** Compute squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSize * gridSize`) instead of `Math.pow` and `Math.sqrt` to eliminate JNI overhead and improve loop performance.

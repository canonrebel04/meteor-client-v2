...
## 2024-06-18 - Avoid Math.pow and Math.sqrt in hot loops
**Learning:** Using `Math.pow` and `Math.sqrt` for distance calculations inside performance-critical paths like nested loops involves heavy native JNI floating-point calculations overhead.
**Action:** Compute the squared distance using primitive integer multiplication (e.g., `dx * dx + dz * dz > gridSize * gridSize`) to eliminate unnecessary floating-point JNI overhead.

...
## 2023-11-20 - Missing tooltip on icon-only remove widget
**Learning:** In the `Marker` module GUI, an icon-only button (`theme.minus()`) lacked a tooltip, making it difficult for screen readers to interpret and confusing for users. Icon-only widgets do not have accessible labels by default.
**Action:** Always ensure that icon-only widgets, such as `WMinus`, have their `.tooltip` explicitly set upon instantiation to improve clarity and accessibility.

...
## $(date +%Y-%m-%d) - Adding Placeholders with `GuiTheme#textBox`
**Learning:** The custom UI framework's `GuiTheme#textBox` method does not have a `textBox(String text, String placeholder)` signature. It relies on the `textBox(String text, String placeholder, CharFilter filter)` signature when placeholders are needed.
**Action:** When adding placeholders to textboxes in `GuiTheme`, always provide a `CharFilter` (e.g., `(textIn, c) -> true`) to match the existing method signatures and prevent compilation errors. Ensure lambdas avoid using the un-named variable `_` in environments that might not have Java 22 preview features enabled.
